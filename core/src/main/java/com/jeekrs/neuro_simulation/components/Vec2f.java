...
public class Vec2f extends Component implements Vector<Vec2f> {
    ...
    public final static Vec2f X = new Vec2f(1, 0);
    public final static Vec2f Y = new Vec2f(0, 1);
     ...
     * @return This vector for chaining
     */
    public Vec2f scl(float x, float y) {
        this.x *= x;
        this.y *= y;
        return this;
    }
    ...
    @Override
    public Vec2f mulAdd(Vec2f vec, float scalar) {
        this.x += vec.x * scalar;
        this.y += vec.y * scalar;
        return this;
    }
    ...
    @Override
    public Vec2f mulAdd(Vec2f vec, Vec2f mulVec) {
    ...
    @Override
    public Vec2f limit2(float limit2) {
        float len2 = len2();
        if (len2 > limit2) {
    ...
    @Override
    public Vec2f setLength(float len) {
        return setLength2(len * len);
    }

    @Override
    public Vec2f setLength2(float len2) {
        float oldLen2 = len2();
        return (oldLen2 == 0 || oldLen2 == len2) ? this : scl((float) Math.sqrt(len2 / oldLen2));
    }

    /**
     * Converts this {@code Vec2f} to a string in the format {@code (x,y)}.
     *
    ...
    /**
     * Sets this {@code Vec2f} to the value represented by the specified string according to the format of {@link #toString()}.
     *
     * @param v the string.
     * @return this vector for chaining
     */
    public Vec2f fromString(String v) {
        int s = v.indexOf(',', 1);
        ...
        throw new GdxRuntimeException("Malformed Vec2f: " + v);
    }
     ...
     */
    public float angle(Vec2f reference) {
     ...
     * (typically counter-clockwise.)
     */
    public float angleRad(Vec2f reference) {
    ...
    /**
     * Rotates the Vec2f by the given angle, counter-clockwise assuming the y-axis points up.
     *
    ...
    /**
     * Rotates the Vec2f by the given angle around reference vector, counter-clockwise assuming the y-axis points up.
     *
     * @param degrees   the angle in degrees
     * @param reference center Vec2f
     */
    public Vec2f rotateAround(Vec2f reference, float degrees) {
     ...
     * @param reference center Vec2f
     */
    public Vec2f rotateAroundRad(Vec2f reference, float radians) {
        return this.sub(reference).rotateRad(radians).add(reference);
    }

    /**
     * Rotates the Vec2f by 90 degrees in the specified direction, where >= 0 is counter-clockwise and < 0 is clockwise.
     */
    ...
    @Override
    public Vec2f lerp(Vec2f target, float alpha) {
    ...
    @Override
    public Vec2f interpolate(Vec2f target, float alpha, Interpolation interpolation) {
        return lerp(target, interpolation.apply(alpha));
    }

    @Override
    public Vec2f setToRandomDirection() {
     ...
     */
    public boolean epsilonEquals(final Vec2f other) {
    ...
    @Override
    public boolean isOnLine(Vec2f other) {
        return MathUtils.isZero(x * other.y - y * other.x);
    }

    @Override
    public boolean isOnLine(Vec2f other, float epsilon) {
        return MathUtils.isZero(x * other.y - y * other.x, epsilon);
    }

    @Override
    public boolean isCollinear(Vec2f other, float epsilon) {
        return isOnLine(other, epsilon) && dot(other) > 0f;
    }

    @Override
    public boolean isCollinear(Vec2f other) {
        return isOnLine(other) && dot(other) > 0f;
    }

    @Override
    public boolean isCollinearOpposite(Vec2f other, float epsilon) {
        return isOnLine(other, epsilon) && dot(other) < 0f;
    }

    @Override
    public boolean isCollinearOpposite(Vec2f other) {
        return isOnLine(other) && dot(other) < 0f;
    }

    @Override
    public boolean isPerpendicular(Vec2f vector) {
        return MathUtils.isZero(dot(vector));
    }

    @Override
    public boolean isPerpendicular(Vec2f vector, float epsilon) {
        return MathUtils.isZero(dot(vector), epsilon);
    }

    @Override
    public boolean hasSameDirection(Vec2f vector) {
        return dot(vector) > 0;
    }

    @Override
    public boolean hasOppositeDirection(Vec2f vector) {
        return dot(vector) < 0;
    }
